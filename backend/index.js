 ...
 * @returns {{ name: string, surename: string, lastname: string, BirthDay: string, Start: string, Facultet: string }} Объект студента
 */
function makeStudentFromData(data) {
    ...
    const student = {
        ...
        Facultet: asString(data.Facultet),
    }
    ...
    if (!student.surename) errors.push({ field: 'surename', message: 'Не указана фамилия' });
    ...
    if (!student.Start) errors.push({ field: 'Start', message: 'Не указано начало обучения' });
    if (!student.Facultet) errors.push({ field: 'Facultet', message: 'Не указан факультет' });
 ...
 * @returns {{ id: string, name: string, surename: string, lastname: string, BirthDay: string, Start: string, Facultet: string }[]} Массив студентов
 */
 ...
 * @returns {{ id: string, name: string, surename: string, lastname: string, BirthDay: string, Start: string, Facultet: string, createdAt: string, updatedAt: string }} Объект студента
 */
 ...
 * @param {{ name?: string, surename?: string, lastname?: string, BirthDay?: string, Start?: string, Facultet?: string }} data - Объект с изменяемыми данными
 * @throws {ApiError} Студент с таким ID не найден (statusCode 404)
 * @throws {ApiError} Некорректные данные в аргументе (statusCode 422)
 * @returns {{ id: string, name: string, surename: string, lastname: string, BirthDay: string, Start: string, Facultet: string, createdAt: string, updatedAt: string }} Объект студента
 */
...
module.exports = createServer(async(req, res) => {
    ...
    .on('listening', () => {
        if (process.env.NODE_ENV !== 'test') {
            ...
            console.log(`POST ${URI_PREFIX} - создать студента, в теле запроса нужно передать объект { name: string, surename: string, lastname: string, BirthDay: string, Start: string, Facultet: string}`);
            console.log(`GET ${URI_PREFIX}/{id} - получить студента по его ID`);
            console.log(`PATCH ${URI_PREFIX}/{id} - изменить студента с ID, в теле запроса нужно передать объект { name?: string, surename?: string, lastname?: string, BirthDay?: string, Start?: string, Facultet?: string}`);
